...
<template>
    <div>
        <div class="min-h-screen bg-background">
            <nav class="bg-card border-b border-border">
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex">
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                                ...
                                <NavLink :href="route('entreprises.index')" :active="route().current('entreprises.*')">
                                    Entreprises
                                </NavLink>
                            </div>
                        </div>
                                ...
                                <Dropdown align="right" width="48">
                                    <template #trigger>
                                        <span class="inline-flex rounded-md">
                                            <button
                                                type="button"
                                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-muted-foreground bg-card hover:text-foreground focus:outline-none transition ease-in-out duration-150"
                                            >
                ...
                <div :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }" class="sm:hidden">
                    <div class="pt-2 pb-3 space-y-1">
                        ...
                        <ResponsiveNavLink :href="route('entreprises.index')" :active="route().current('entreprises.*')">Entreprises</ResponsiveNavLink>
                    </div>
                    <div class="pt-4 pb-1 border-t border-border">
                        <div class="px-4">
                            <div class="font-medium text-base text-foreground">{{ $page.props.auth.user.name }}</div>
            ...
            <!-- Page Heading -->
            <header class="bg-card shadow-sm" v-if="$slots.header">
